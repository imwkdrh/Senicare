...
import {  HR_ABSOLUTE_PATH } from 'src/constants';
import { getChargedCustomerRequest, getNurseRequest, patchNurseRequest } from 'src/apis';
import { GetNurseResponseDto } from 'src/apis/dto/response/nurse';
import { ResponseDto } from 'src/apis/dto/response';
import { usePagination } from 'src/hooks';
import Pagination from 'src/components/Pagination';
import { ChargedCustomer } from 'src/types';
import { GetChargedCustomerResponseDto } from 'src/apis/dto/response/customer';
...
export default function HRDetail() {
    ...
    const getChargedCustomerResponse = (responseBody: GetChargedCustomerResponseDto | ResponseDto | null) => {
        const message = !responseBody ? '서버에 문제가 있습니다.' :
            responseBody.code === 'AF' ? '잘못된 접근입니다.' :
                responseBody.code === 'DBE' ? '서버에 문제가 잇습니다.' : '';
    ...
    // effect: 요양사 아이디가 변경 될 때 실행할 요양사 정보 불러오기 함수//
    useEffect(() => {
        if (!userId) return;
        const accessToken = cookies[ACCESS_TOKEN];
        if (!accessToken) return;
    ...
    // effect: update 상태가 변경될 때 실행할 함수//
    useEffect(() => {
        setUpdateName(name);
    },[update,name]);


    // render : 인사 정보 상세보기 컴포넌트 렌더링 //
    return (
        <div id='hr-detail-wrapper'>
            ...
            <div className='bottom'>
                ...
                <div className='button primary' onClick={onShowUpdateClickHandler}>수정</div>
                )}
            </div>
        </div>
    )
