...
import { ACCESS_TOKEN, HR_DETAIL_ABSOLUTE_PATH, HR_DETAIL_PATH } from 'src/constants';
...
export default function HR() {
    ...
    const getNurseListResponse = (responseBody: GetNurseListResponseDto | ResponseDto | null) => {
        const message = !responseBody ? '서버에 이상이 있습니다.' :
            responseBody.code === 'AF' ? '잘못된 접근입니다.' :
                responseBody.code === 'DBE' ? '서버에 이상이 있습니다.' : '';
    ...
    const onSearchWordChange = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        const searchedNurses = originalList.filter(nurse => nurse.name.includes(value));
        setTotalList(searchedNurses);
        initViewList(searchedNurses);
    }

    // event handler: 검색 번튼 클릭 이벤트 처리 //
        ...
        <div id='hr-wrapper'>
            ...
            <div className='bottom'>
                ...
                <div className='search-box'>
                    <input className='search-input' value={searchWord} placeholder='검색어를 입력하세요.' onChange={onSearchWordChange} />
