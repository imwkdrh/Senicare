...
import { ACCESS_TOKEN, CS_ABSOLUTE_PATH, CS_DETAIL_ABSOLUTE_PATH, CS_UPDATE_ABSOLUTE_PATH, CS_WRITE_ABSOLUTE_PATH } from 'src/constants';
import { deleteCustomerRequest, getCustomerListRequest } from 'src/apis';
import GetCustomerListResponseDto from './../../apis/dto/response/customer/get-customer-list.response.dto';
...
function TableRow({ customer, getCustomerList }:TableRowProps) {
    ...
    const onUpdateButtonsClickHandler = (event: MouseEvent<HTMLDivElement>) => {
            ...
            {isCharger &&
                <div className='td-buttons'>
                    <div className='td-edit'>
                        <div className='icon-button edit' onClick={onUpdateButtonsClickHandler}></div>
                    </div>
...
export default function CSUpdate() {
    ...
    // state: 페이징 관련 상태 //
    const { currentPage, totalPage, totalCount, viewList, setTotalList, initViewList, ...paginationProps } = usePagination<Customer>();
    ...
    const onSearchButtonClickHandler = () => {
        const searchedCustomList = originalList.filter(customer => customer.name.includes(searchWord));
        setTotalList(searchedCustomList);
        initViewList(searchedCustomList);
    };
