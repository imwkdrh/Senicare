...
import GetCareRecordResponseDto from 'src/apis/dto/response/customer/get-care-record-list.response.dto';
import GetToolListResponseDto from './../../../apis/dto/response/tool/get-tool-list.response.dto';
import PostCareRecordRequestDto from './../../../apis/dto/request/customer/post-care-record-.request.dto';
import Pagination from 'src/components/Pagination';



// component: 고객 정보 상세보기 컴포넌트//
export default function CSDetail() {
    ...
    // state: 페이징 관련 상태 //
    const { currentPage,totalPage,totalCount,viewList,setTotalList,initViewList, ...paginationProps} = usePagination<CareRecord>();

    // state: 용품 상태 셀렉터 오픈 여부 상태 //
    ...
    // effect: 고객 번호가 바뀔 때 고객 정보 요청 함수 //
    useEffect(() => {
        if (!customerNumber) return;
        const accessToken = cookies[ACCESS_TOKEN];
        if (!accessToken) return;
