...
import {  GetCareRecordListResponseDto, GetChargedCustomerResponseDto, GetCustomerListResponseDto, GetCustomerResponseDto } from "./dto/response/customer";
...
const GET_CHARGED_CUSTOMER_API_URL = (nurseId: string) => `${NURSE_MODULE_URL}/${nurseId}/customers`
...
const PATH_CUSTOMER_API_URL = (customerNumber: number | string) => `${CUSTOMER_MODULE_URL}/${customerNumber}`;
...
export const getNurseListRequest = async (accessToken: string) => {
    const responseBody = await axios
        .get(GET_NURSE_LIST_API_URL, bearerAuthorization(accessToken))
...
export const getSignInRequest = async (accessToken: string) => {
    const responseBody = await axios
        .get(GET_SIGN_IN_API_URL, bearerAuthorization(accessToken))
...
export const patchCustomerRequest = async (requestBody: PatchCustomerRequestDto, customerNumber: number | string, accessToken: string) => {
    const responseBody = await axios.patch(PATH_CUSTOMER_API_URL(customerNumber),requestBody, bearerAuthorization(accessToken))
        .then(responseDataHandler<ResponseDto>)
        .catch(responseErrorHandler);
    return responseBody;
}
